...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens_packed)  # (src_len, b, h*2)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # (b, src_len, h*2)
        # init_decoder_hidden
        last_hidden_cat = torch.cat((last_hidden[0], last_hidden[1]), dim=1)  # (b, 2*h)
        init_decoder_hidden = self.h_projection(last_hidden_cat)  # (b, h)
        # init_decoder_cell
        last_cell_cat = torch.cat((last_cell[0], last_cell[1]), dim=1)  # (b, 2*h)
        init_decoder_cell = self.c_projection(last_cell_cat)  # (b, h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim=2)).squeeze(2)
        ### END YOUR CODE
