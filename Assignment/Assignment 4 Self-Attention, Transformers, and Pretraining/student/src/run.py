...
def main():
    ...
    if args.function == 'pretrain':
        ...
        ### YOUR CODE HERE ###
        # Pretraining dataset is already created above as pretrain_dataset
        ...
        tr = trainer.Trainer(model, pretrain_dataset, None, tconf)
        tr.train()

        torch.save(model.state_dict(), args.writing_params_path)
        print(f"Saved pretrained model parameters to {args.writing_params_path}")
        ...
        ### YOUR CODE HERE ###
        # Build finetuning dataset (NameDataset expects a single string)
        ...
        if args.reading_params_path is not None:
            # In this codebase, `device` may be an int (CUDA device index).
            # torch.load(map_location=...) cannot take an int, so convert.
            if isinstance(device, int):
                map_loc = torch.device(f"cuda:{device}")
            else:
                map_loc = device  # 'cpu', 'mps', or torch.device
            state = torch.load(args.reading_params_path, map_location=map_loc)
            model.load_state_dict(state)

        # Switch epochs based on whether we loaded a pretrained model
        ...
        tr = trainer.Trainer(model, finetune_dataset, None, tconf)
        tr.train()

        torch.save(model.state_dict(), args.writing_params_path)
        print(f"Saved finetuned model parameters to {args.writing_params_path}")
