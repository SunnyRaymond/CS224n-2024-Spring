...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    attention_mask: [bs, 1, 1, seq_len] (0 for tokens, large negative for padding)
    return: [bs, seq_len, hidden_size]
    """
    # 1) raw attention scores: [bs, heads, L, L]
    scores = torch.matmul(query, key.transpose(-1, -2))

    # 2) scale by sqrt(head_dim)
    scores = scores / (self.attention_head_size ** 0.5)

    # 3) mask padding positions (broadcast add)
    scores = scores + attention_mask

    # 4) softmax -> attention probs
    attn_probs = F.softmax(scores, dim=-1)

    # 5) dropout on attention probs
    attn_probs = self.dropout(attn_probs)

    # 6) weighted sum over values: [bs, heads, L, head_dim]
    context = torch.matmul(attn_probs, value)

    # 7) concat heads: [bs, L, heads, head_dim] -> [bs, L, hidden]
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    output: [bs, seq_len, hidden]  (from attention or from FFN)
    """
    # 1) linear projection
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # 3) feed-forward (intermediate)
    interm_output = self.interm_dense(hidden_states)
    interm_output = self.interm_af(interm_output)

    # 4) add & norm after FFN (note: add_norm will apply out_dense inside)
    hidden_states = self.add_norm(
        input=hidden_states,
